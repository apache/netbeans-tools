pipeline {
   ...
   stages {
      stage('Informations') {
          steps {
              slackSend (channel:'#netbeans-builds', message:"STARTED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' ($env.BUILD_URL), Branch we are building is : refs/heads/release90",color:'#f0f0f0')
          }
      }
      stage('SCM operation') {
          steps {
