...
$url = 'https://raw.githubusercontent.com/apache/netbeans-jenkins-lib/master/meta/netbeanshtml4j.json';
...
$extract_dir = '/var/www/bits.netbeans.org/html+java';
...
if ($json) {
    ...
    // The JSON is an object where keys are tag names.
    // Each entry in the object has a 'version_name' with version information.
    // Target url is of the form https://bits.netbeans.org/html+java/${version_name}/
    if ($data) {
        exec('mkdir -p ' . $output_dir);
        exec('mkdir -p ' . $extract_dir);
        foreach (array_keys($data) as $branch_name) {
            $release_info = $data[$branch_name];
            $version_name = $release_info['version'];
            $release_extract_dir = $extract_dir . '/' . $version_name . '/';
            $netbeans_javadoc_zip_url = "https://builds.apache.org/job/Netbeans/job/netbeans-html4j-TLP/job/netbeans-html4j/job/" .
                $branch_name .
                "/lastSuccessfulBuild/artifact/WEBSITE.zip";
	    
            $longapidocurl = $release_info['apidocurl'];
	    $apidocurl = substr($longapidocurl,26); // remove bits.netbeans.org	    

	    $index = '<li>';
	    $index .= '<a href="'.$longapidocurl.'">Apache NetBeans HTML4J ';
	    ...
	    $index .= "</li>";
	    if ($release_info['position']>4) {
	       $indexa[$release_info['position']] = $index;
	    }
            $netbeans_javadoc_zip_file = $output_dir . '/netbeans-html4j-javadoc-' . $version_name . '.zip';

            /* Download using 'download-if-required.sh' */
            $command = $download_if_required . ' ' . $netbeans_javadoc_zip_url . ' ' . $netbeans_javadoc_zip_file;
            echo "Downloading javadoc zip for html4j release ", PHP_EOL;
		...
		if ($return_val != 0) {
                ...
                /* rename  apidocs to extractdir*/
		$cmd = 'mv'.' '.$release_extract_dir.'apidocs/*'.' '.$release_extract_dir.'/';
		exec($cmd, $command_out, $return_val);
		if ($return_val != 0) {
		    echo "Could no move apidocs files", PHP_EOL;
		    echo implode(PHP_EOL,$command_out);
		}
            } else {
	...
	$findex = $output_dir .'/jdochtml4jindex.txt';
